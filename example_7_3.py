...
class Cell:
    ...
    def __sub__(self, other):
        return Cell(self.cnt - other.cnt) if (self.cnt - other.cnt) > 0 else print('Недопустимая операция!')
    ...
    def __truediv__(self, other):
        return Cell(floor(self.cnt / other.cnt))

    def make_order(self, cells_in_row):
        row = ''
        for i in range(int(self.cnt / cells_in_row)):
            row += f'{"*" * cells_in_row}\\n'
        if self.cnt % cells_in_row == 0:
            row = row[:len(row) - 2]
        else:
            row += f'{"*" * (self.cnt % cells_in_row)}'
        return row

    def __str__(self):
        return f'Количество клеток {self.cnt}: {self.cnt * "*"}'
